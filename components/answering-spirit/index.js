...
const AnsweringSpirit = props => {
  ...
  const changeGenra = async(newGenre) => {
    await props.setGenre(newGenre);
  }

  //write the finalizeAnswer function here
  const finalizeAnswer = async() => {
    await props.ChangeGenre("");
    ...
    return (
        <View style={props.styles.aoLobbyContainer}>
          ...
          <View style={{...props.styles.aoSpiritWatchingGameBar, backgroundColor: "#000000"}}>
            ...
            <TouchableOpacity style={selectedLetter === "" ? props.styles.aoSecondaryButton : props.styles.aoSecondaryButtonDisabled} onPress={async() => await finalizeAnswer()}>
